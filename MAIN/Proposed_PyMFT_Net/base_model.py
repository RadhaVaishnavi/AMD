...
def shortcut(x, y, mode="projection"):
    '''
    shortcut connection
...
def basic_block(filters, first_strides, is_first_block_of_first_layer, mode="projection"):
    '''
...
def basic_block_pyramid(filters, first_strides, is_first_block_of_first_layer, mode="zero-padding"):
    '''
    provide bulding block func
    ...
    def f(x):
        if is_first_block_of_first_layer:
            ...
            conv1 = bn_conv(filters=filters, kernel_size=(3, 3),
                                 strides=first_strides)(x)

        conv2 = bn_relu_conv(filters=filters, kernel_size=(3, 3))(conv1)
        bn=BatchNormalization()(conv2)
        return shortcut(x,bn, mode=mode)
    return f
...
def bottleneck_block(filters, first_strides, is_first_block_of_first_layer, mode="zero-padding"):
    ...
    return f
def bottleneck_block_pyramid(filters, first_strides, is_first_block_of_first_layer, mode="zero-padding"):
    '''
    provide bottleneck bulding block func
    ...
    def f(x):
        ...
        conv2 = bn_relu_conv(filters=filters, kernel_size=(3, 3))(conv1)
        conv3 = bn_relu_conv(filters=filters*outchannel_ratio, kernel_size=(1, 1))(conv2)
...
class ResnetBuilder():
    ...
    def build(input_shape, num_outputs, block_type, repetitions, wide=False, mode="projection"):
        ...
        input_shape
        num_outputs
        block_type : residual block('basic' or 'bottleneck')
        repetitions: 
        wide:
        mode:
        '''
...
class PyramidNetBuilder():
    @staticmethod
    def build(input_shape, num_outputs, block_type,alpha=240,depth=110,mode="zero-padding"):
        '''
        PyramidNet 
        ...
        # block_type 
        if block_type == 'basic':
            block_fn = basic_block_pyramid
            n=int((depth-2)/6)
        elif block_type == 'bottleneck':
            block_fn = bottleneck_block_pyramid
            n=int((depth-2)/9)
        repetitions=[n,n,n]
        num_blocks = np.sum(repetitions)
        filters_list = np.floor(np.linspace(16,16+alpha,num_blocks+1)).astype('int')
